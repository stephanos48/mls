...
namespace mls.Controllers
{
    ...
    public class PfepsController : Controller
    {
        ...
        // GET: Pfeps/StockOut
        [HttpGet]
        public ActionResult _UsaStockOut(int customer, int division)
        {
            ...
            var queryNew = from mp in db.MasterPartLists
                           ...
                           where mp.CustomerId == customer && mp.CustomerDivisionId == division && mp.ActivePartId == 1 && tx.Qoh == 0
        ...
        // GET: Pfeps/LowStock
        [HttpGet]
        public ActionResult _UsaLowStock(int customer, int division)
        {
            ...
            var queryNew = from mp in db.MasterPartLists
                           ...
                           join tx in db.TxQohs on mp.CustomerPn equals tx.Pn
                           join po in db.PurchaseOrders.Where(r => r.PoOrderStatusId != 5) on mp.CustomerPn equals po.CustomerPn into pogroup
                           where mp.CustomerId == customer && mp.CustomerDivisionId == division && mp.ActivePartId == 1 && tx.Qoh <= pfep.Min.Value && tx.Qoh != 0
                           orderby mp.CustomerPn
                           select new
                           {
                               mp.CustomerPn,
        ...
        // GET: Pfeps/LowStock
        [HttpGet]
        public ActionResult _UsaNeedToOrder(int customer, int division)
        {
            ...
            var queryNew = from mp in db.MasterPartLists
                           ...
                           join tx in db.TxQohs on mp.CustomerPn equals tx.Pn
                           join po in db.PurchaseOrders.Where(r => r.PoOrderStatusId != 5) on mp.CustomerPn equals po.CustomerPn into pogroup
                           where mp.CustomerId == customer && mp.CustomerDivisionId == division && mp.ActivePartId == 1 && tx.Qoh <= pfep.PfepTx
                           orderby mp.CustomerPn
                           select new
                           {
                               mp.CustomerPn,
        ...
        // GET: Pfeps/AllParts
        [HttpGet]
        public ActionResult _UsaAllParts(int customer, int division)
        {
            ...
            var queryNew = from mp in db.MasterPartLists
                           ...
                           join tx in db.TxQohs on mp.CustomerPn equals tx.Pn
                           join po in db.PurchaseOrders.Where(r => r.PoOrderStatusId != 5) on mp.CustomerPn equals po.CustomerPn into pogroup
                           where mp.CustomerId == customer && mp.CustomerDivisionId == division && mp.ActivePartId == 1
                           orderby mp.CustomerPn
                           select new
                           {
                               mp.CustomerPn,
